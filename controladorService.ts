...
export class ControladorService<T extends ColeccionItem> {
  ...
  public readonly total = computed(() => {
    const items = this._coleccion() as (T & ItemCalculable)[];
    return items.reduce((acc, item) => {
      if (item.price && item.cantidad) {
  ...
  public readonly totalItems = computed(() => {
    return this._coleccion().length;
  });

  agregarItem(item: T): void {
    this._coleccion.update(coleccion => [...coleccion, item]);
  }

  eliminarItem(id: string | number): void {
    this._coleccion.update(coleccion => coleccion.filter(i => i.id !== id));
  }
  ...
  buscarItemPorId(id: string | number): T | undefined {
    return this._coleccion().find(item => item.id === id);
  }

  async guardarColeccion(nombreColeccion: string, idDocumento: string): Promise<void> {
    this.states.set(StatesEnum.LOADING);
    try {
      const datos = { items: this._coleccion() };
      await this.writeService.crearDocumento(nombreColeccion, idDocumento, datos);
  ...
  async cargarColeccion(nombreColeccion: string, idDocumento: string): Promise<void> {
    this.states.set(StatesEnum.LOADING);
    try {
      const documento = await this.readService.leerDocumento(nombreColeccion, idDocumento);
      if (documento && documento['items']) {
        this._coleccion.set(documento['items']);
      }
      this.states.set(StatesEnum.SUCCESS);
    } catch (error) {
      this.states.set(StatesEnum.ERROR);
